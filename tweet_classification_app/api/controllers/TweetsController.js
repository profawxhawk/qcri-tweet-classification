	...
	mainf: function(req, res) { // finding all the labels associated with the category
		...
		Sim.native(function(err, coll) {
			coll.distinct("image_damage_class", {
				"code": req.param('code')
			}, function(err, result) {  //get all distinct values of labels for image damage class
				...
				coll.distinct("aidr_class_label", {
					"code": req.param('code')
				}, function(err, result1) {    //get all distinct values of labels for aidr 
					...
					coll.distinct("sentiment", {
						"code": req.param('code')
					}, function(err, result2) {    //get all distinct values of labels for sentiment
						...
						coll.distinct("image_physical_location", {
							"code": req.param('code')
						}, function(err, result3) {   //check whether there are entries with images or not
							console.log('j');
							if (result3.length == 0) {
								respose.push(0);
